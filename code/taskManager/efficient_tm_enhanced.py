...
idle_power_values = [11.2, 5.4, 2.1, 1.9, 2.3]  # Idle Power Values -> you have to collect them beforehand
...
def distribute_tasks_by_efficiency():
    ...
    for client_id in sorted_clients:
        ...
        new_data = pd.DataFrame([{
            ...
            "inv_efficiency": 1 / float(efficiency) if efficiency > 0 else float('inf')  # Nicole ergänzt, Verhindert Division durch 0
        }])
...
def distribute_tasks_to_clients(client, task_distribution):
    ...
    for client_id, tasks in task_distribution.items():
        if client_id in connected_clients:
            ...
            print(f"Sent: {len(tasks)} tasks to {client_id} via topic: {topic} at {current_timestamp}.") #Nicole
...
def on_message(client, userdata, msg):
    ...
    if topic.startswith("finish/"):
        ...
        if clients_with_tasks == finisher_counter:
            ...
            duration = stop_distribution_timestamp - start_distribution
